...
module.exports = async function gtmModule (_options) {
  ...
  if (typeof this.options.head === 'function') {
    // eslint-disable-next-line no-console
    console.warn(
      '[@moosesaid/gtm] head is provided as a function which is not supported by this module at the moment. Removing user-provided head.'
    )
    this.options.head = {}
  }
  ...
  if (options?.type === 'text/partytown') {
    scriptTag.type = options.type
  }

  this.options.head.script.push(scriptTag)
  ...
  this.options.head.__dangerouslyDisableSanitizersByTagID[options.type] = [
    'innerHTML'
  ]
